...
abstract class BaseCommand extends Command
{
    ...
    {
        return config('ac-developers.generator.rootNamespace', $this->laravel->getNamespace());
    }
    ...
    protected function getPath($name)
    {
        ...
        return config('ac-developers.generator.basePath', $this->laravel['path']).'/'.str_replace('\\', '/', $name).'.php';
    }
    ...
    {
        $modelClass = $this->parseClass($this->option('model'), config('ac-developers.generator.namespaces.models', ''));
        ...
        if (! class_exists($modelClass)) {
            ...
            if ($this->confirm("A {$modelClass} model does not exist. Do you want to generate it?", true)) {
                $this->call('ac:model', ['name' => $modelClass]);
            }
